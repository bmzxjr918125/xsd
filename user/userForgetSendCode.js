define(function(require){
	...
	Model.prototype.getCodeBtnClick = function(event){
	  ...
	  if( !common.checkPhone(phoneNumber)){
		...
		$.ajax({
			...
			"dataType" : "jsonp",
			"async" : false,
			"url" : common.apiUrl+"clientUserApiGetForgetCode?phoneNumber="+phoneNumber,
			"jsonp" : "callback",
    ...
    //找回密码跳转密码重置页面
	Model.prototype.forGetCodeClick = function(event){
	    var phoneNumber = this.comp("phoneNumber").val();
	   ...
	   if( !common.checkPhone(phoneNumber)){
		...
		$.ajax({
			...
			"dataType" : "jsonp",
			"async" : false,
			"url" : common.apiUrl+"clientUserApiValiForGetCode?phoneNumber="+phoneNumber+"&phoneCode="+phoneCode,
			"jsonp" : "callback",
			...
			"success" : function(resultData) {
				...
				if (resultData.code == "400") {
				     ...
				     common.removeLoadWait();
				     common.loadWait("正在跳转密码重置页面，请稍后..");
				     setTimeout(function(){
		    		 common.toForget();
		    		 }, 1000);
				}
